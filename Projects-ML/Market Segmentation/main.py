...
#importing the model
file_name = 'final_model_dt.sav'
model = pickle.load(open(file_name,'rb'))

#loading the data
df = pd.read_csv('Clustered_Customer_Data1.csv')
st.set_option('deprecation.showPyplotGlobalUse', False)

st.markdown('<style>body{background-color: Blue;}</style>',unsafe_allow_html=True)
st.title("Market Segmentation")
st.markdown("Market segmentation involves dividing a large homogenous market of \
...
with st.form('my_form'):
    ...
    onoff_purchases = st.number_inut(label='OnOff Purchases',step=0.01,format="%.2f")
...
if submitted:
    clust = model.predict(data)[0]
    print('Data belongs to Cluster',clust)
    st.text('Belongs to Cluster',clust)
